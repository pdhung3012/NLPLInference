...
public class AnalyzeCMForLibrary {

	public static String[] arr6LibPrefix={"android","com.google.gwt","com.thoughtworks.xstream","org.hibernate","org.joda.time","java"};
	...
	public static void main(String[] args) {
			...
			for(int indexLib=0;indexLib<arr6LibPrefix.length;indexLib++){
				String fopLibEval=fopEval+arr6LibPrefix[indexLib]+File.separator;
				new File(fopLibEval).mkdir();
				String fpStatIdenByNumCSV=fopLibEval+"statCorrectNum.csv";
				String fpStatIdenBySetCSV=fopLibEval+"statCorrectSetTarget.csv";		
				String fpStatLineCSV=fopLibEval+"statCorrectLine.csv";
				String fpStatReceiverCSV=fopLibEval+"statCorrectReceiver.csv";
				String fpStatIdenBySetTXT=fopLibEval+"statCorrectSetTarget.txt";		
				String fpStatLineTXT=fopLibEval+"statCorrectLine.txt";
				String fpStatReceiverTXT=fopLibEval+"statCorrectReceiver.txt";
				String fpStatSourceTargetCSV=fopLibEval+"statST.csv";
				String fpStatSourceTargetTXT=fopLibEval+"statST.txt";
...
//				3.find correct with maximum question marks
					...
					for(int j=0;j<arrItemSource.length;j++){
						if(arrItemTarget[j].startsWith("E-Total")){
							if( arrItemTarget[j].equals(arrItemTrans[j])){
								...
								if(!libItem.startsWith(arr6LibPrefix[indexLib])){
									continue;
								}
				...
				System.out.println("end lib "+arr6LibPrefix[indexLib]);
			}
