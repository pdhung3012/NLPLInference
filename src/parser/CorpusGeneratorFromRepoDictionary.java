...
public class CorpusGeneratorFromRepoDictionary {
private String repoPath;
	
	public static void main(String[] args) {
		long start = System.currentTimeMillis();
//		specify the specific lib you want to use.
		String[] libs = new String[]{"org.apache.commons.", "android.", "com.google.gwt.", "org.hibernate.", "org.joda.time.", "com.thoughtworks.xstream."};
		CorpusGeneratorFromRepoDictionary cg = new CorpusGeneratorFromRepoDictionary("your folder of all projects");
		cg.generateSequences("folder of repository names for each lib (store in csv files)", false, libs, "your folder of extracted sequences");
		long end = System.currentTimeMillis();
		System.out.println("Finish parsing corpus in " + (end - start) / 1000);
	}

	public CorpusGeneratorFromRepoDictionary(String repoPath) {
	...
	public void generateSequences(final String repoListsPath,final boolean keepUnresolvables, final String[] libs, final String outPath) {
		for (final String lib : libs) {
			new Thread(new Runnable() {
				@Override
				public void run() {
					String content = FileUtil.getFileContent(repoListsPath  + lib + ".csv");
